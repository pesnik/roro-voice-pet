...
// Reading ~/.codex/session_index.jsonl for a friendlier Codex thread name
// lived in hooks/codex-session-index.js, which only makes sense alongside
// the Codex CLI hook integration this app doesn't have. No Codex session
// will ever reach here (the only /state poster is our own MiniCPM sidecar,
// which always identifies as agent_id "claude-code"), so this degrades to
// "no thread name found" rather than a real lookup.
function readCodexThreadName() { return null; }
