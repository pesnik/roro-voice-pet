...
function locateSidecarBinary(appRoot) {
  ...
  const devBin = path.join(appRoot, "..", "sidecar", "bin", triple, "minicpm-sidecar" + ext);
...
function locateSidecarSourceDir(appRoot) {
  ...
  if (app && app.isPackaged) {
    ...
    candidates.push(path.join(process.resourcesPath, "sidecar"));
  }
  candidates.push(path.join(appRoot, "..", "sidecar"));
  for (const c of candidates) {
    try {
