...
test("settings IPC registers owned channels and leaves animation override channels to their module", () => {
  ...
  assert.ok(!ipcMain.handlers.has("settings:list-agents"));
  ...
  assert.ok(!ipcMain.handlers.has("settings:mobile-connection-info"));
...
test("settings IPC lists themes with the active flag set", async () => {
  ...
  assert.deepStrictEqual(await ipcMain.invoke("settings:list-themes"), [
    { id: "clawd", name: "Clawd", active: false },
    { id: "imported-pet", name: "Imported Pet", active: true },
  ]);
});
