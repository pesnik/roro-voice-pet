...
describe("applyCommand", () => {
  ...
  it("applies a command's commit without clobbering unrelated fields", async () => {
    const ctrl = createSettingsController({
      prefsPath: makeTempPath(),
      loadResult: {
        snapshot: { ...prefs.getDefaults(), soundMuted: true, lang: "zh" },
        locked: false,
      },
      commands: {
        muteSound: () => ({ status: "ok", commit: { soundMuted: false } }),
      },
    });
    const r = await ctrl.applyCommand("muteSound", null);
    assert.strictEqual(r.status, "ok");
    assert.strictEqual(ctrl.get("soundMuted"), false);
    assert.strictEqual(ctrl.get("lang"), "zh");
  });
