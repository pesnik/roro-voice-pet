...
describe("buildSessionSnapshot", () => {
  ...
  it("builds renderer-safe fields, groups, and both dashboard/menu orderings", () => {
    ...
    // No per-agent icon badge in this app (see state-agent-icons.js removal) —
    // getAgentIconUrl is never supplied, so iconUrl is always null.
    assert.strictEqual(oldWorking.iconUrl, null);
...
describe("Stop completion gate (#406)", () => {
  ...
  // Claude Code transcript-tail reading (promoting completion by reading the
  // hook's transcript.jsonl when Stop never arrives) lived in
  // hooks/clawd-hook.js, which only exists alongside the Claude Code hook
  // script this app doesn't have. state.js now stubs that reader to always
  // return "no output found" (see src/state.js), so the probe below never
  // promotes — covered by the two negative-path tests that follow, which
  // still hold since our sidecar never sends transcript_path/AskUserQuestion
  // anyway.
